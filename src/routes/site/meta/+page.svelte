      ...
      {#if siteInfo.technologyStack && siteInfo.technologyStack.length > 0}
        ...
        <ul>
          {#each siteInfo.technologyStack as tech}
            <li>
              {#if tech?.url}
                <a href={tech.url} class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
                  {tech?.name}
                </a>
              {:else}
                {tech?.name}
        ...
        {#if siteInfo.openSourceInfo?.basedOn && siteInfo.openSourceInfo.basedOn.length > 0}
          ...
          <ul>
            {#each siteInfo.openSourceInfo.basedOn as item}
              <li>
                {#if item?.url}
                  <a href={item.url} class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
                    {item?.name}
                  </a>
                {:else}
                  {item?.name}
        ...
        {#if siteInfo.openSourceInfo?.repositories && siteInfo.openSourceInfo.repositories.length > 0}
          ...
          <ul>
            {#each siteInfo.openSourceInfo.repositories as repo}
              <li>
                {#if repo?.url}
                  <a href={repo.url} class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
        ...
        {#if siteInfo.openSourceInfo?.license}
          <p>
            ...
            {#if siteInfo.openSourceInfo.license?.url}
              <a href={siteInfo.openSourceInfo.license.url} class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
        ...
        {#if siteInfo.openSourceInfo?.relatedServices && siteInfo.openSourceInfo.relatedServices.length > 0}
          ...
          <ul>
            {#each siteInfo.openSourceInfo.relatedServices as service}
              <li>
                {#if service?.url}
                  <a href={service.url} class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
                    {service?.name}
                  </a>
                {:else}
                  {service?.name}
      ...
      {#if siteInfo.credits && siteInfo.credits.length > 0}
        ...
        <ul>
          {#each siteInfo.credits as credit}
            <li>
              {#if credit?.url}
                <a href={credit.url} class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
                  {credit?.name}
                </a>
              {:else}
                {credit?.name}
              ...
              {#if credit?.license}
                ...
                {#if credit.license?.url}
                  <a href={credit.license.url} class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
      ...
      {#if siteInfo.additionalInfo}
        ...
        {#if siteInfo.additionalInfo?.contact}
          ...
          {#if siteInfo.additionalInfo.contact?.email}
            <p>
              Email:
              <a href={"mailto:" + siteInfo.additionalInfo.contact.email}
                class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
                {siteInfo.additionalInfo.contact.email}
              </a>
            </p>
          {/if}
          {#if siteInfo.additionalInfo.contact?.social && siteInfo.additionalInfo.contact.social.length > 0}
            ...
            <ul>
              {#each siteInfo.additionalInfo.contact.social as social}
                <li>
                  {#if social?.url}
                    <a href={social.url} class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
                      {social?.platform}
                    </a>
                  {:else}
                    {social?.platform}
        ...
        {#if siteInfo.additionalInfo?.sectionLicense && siteInfo.additionalInfo.sectionLicense.length > 0}
          ...
          <ul>
            {#each siteInfo.additionalInfo.sectionLicense as license}
              <li>
                {#if license?.url}
                  <a href={license.url} class="text-[var(--link-color)] hover:text-[var(--link-hover-color)]">
