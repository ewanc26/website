/**
 * Shared seasonal colour calculations.
 *
 * This module is intentionally client-safe so both the server-generated site theme
 * and interactive previews use exactly the same Sabbat interpolation logic.
 */
...
export type ThemeColourName = keyof Baseline;
export type ThemeMode = keyof ModeColors;
...
export function getTargetHues(now: Date): [number, number, number] {
  const { prev, next, progress } = getSabbatContext(now);

  const getSabbatHues = (s: Sabbat): [number, number, number] => {
    const colourful = s.colors
      .map((c) => chroma(c))
...
export function getThemeHue(now: Date, name: ThemeColourName): number {
  const [primaryHue, accentHue, secondaryHue] = getTargetHues(now);

  if (name === "accent") return accentHue;
  if (name === "secondary") return secondaryHue;
  return primaryHue;
}

export function getThemeShade(
  now: Date,
  name: ThemeColourName,
  step: string | number,
  mode: ThemeMode = "light",
): string {
  const data = baseline[name][String(step)];
  if (!data) throw new Error(`Unknown ${name} shade: ${step}`);

  const [lightness, chromaAmount] = data[mode];
  return chroma.oklch(lightness, chromaAmount, getThemeHue(now, name)).hex();
}

export function getCurrentPrimaryShade(step = 400): string {
  try {
    return getThemeShade(new Date(), "primary", step, "light");
  } catch {
    return "#68b34d";
  }
}
