<script lang="ts">
  ...
  let fediverseCreator: string | undefined = import.meta.env.PUBLIC_ACTIVITYPUB_USER;

  $: mastodonUserTag = fediverseCreator && (fediverseCreator.startsWith('http://') || fediverseCreator.startsWith('https://')) ? fediverseCreator : (fediverseCreator && fediverseCreator.startsWith('@') ? fediverseCreator : `@${fediverseCreator}`);

  // Define specific share texts for each platform
  $: blueskyShareText = `${title} by @${profile?.handle} - ${$page.url.href}`;
  $: mastodonShareText = mastodonUserTag && (mastodonUserTag.startsWith('http://') || mastodonUserTag.startsWith('https://')) ? `${title} by ${mastodonUserTag} - ${$page.url.href}` : `${title} by ${mastodonUserTag} - ${$page.url.href}`;
  ...
  $: truncatedMastodonText = mastodonShareText && (mastodonShareText.length > 500 ? mastodonShareText.substring(0, 497) + '...' : mastodonShareText);
  ...
  $: encodedMastodonText = mastodonShareText && encodeURIComponent(truncatedMastodonText);
  ...
  $: mastodonShareUrl = mastodonShareText && `https://${mastodonInstance}/share?text=${encodedMastodonText}`;
