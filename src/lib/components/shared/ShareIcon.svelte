<script lang="ts">
  ...
  $: shareText = `${title} by ${profile?.displayName || data.profile?.handle} on Bluesky (https://bsky.app/profile/${profile?.handle || data.profile?.handle}) - ${$page.url.href}`;
  $: blueskyShareText = `${title} by @${profile?.handle || data.profile?.handle} - ${$page.url.href}`;

  // Encode the share texts for use in URLs
  $: encodedShareText = encodeURIComponent(shareText);
  $: encodedBlueskyText = encodeURIComponent(blueskyShareText);
  ...
  $: mastodonShareUrl = `https://${mastodonInstance}/share?text=${encodedShareText}`;

</script>

<div class={`share-icons flex items-center gap-2 ${showInHeader ? 'ml-auto mr-2' : 'justify-center my-4'}`}>
