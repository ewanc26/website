	...
	{:else if supporters.length > 0}
		<Card variant="elevated" padding="md">
			{#snippet children()}
							...
							<div class="flex flex-col">
								...
								{:else}
									<p class="text-sm text-ink-900 dark:text-ink-100">
										...
										<span class="text-ink-600 dark:text-ink-400">
											{GITHUB_ACTION_LABELS[event.action]}</span
										>
										<span class="ml-1 text-xs text-ink-500 dark:text-ink-500"
											>· {event.tierName} · via GitHub</span
										>
									</p>
								{/if}
								<time
				...
				<div
					class="mt-4 flex flex-wrap gap-x-4 gap-y-2 border-t border-canvas-200 pt-4 dark:border-canvas-700"
				>
