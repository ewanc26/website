<script lang="ts">
    ...
    const ogImage = $derived(image ?? `${page.url.origin}/og/generate?title=${encodeURIComponent(title ?? SITE.title)}${ogType ? `&type=${encodeURIComponent(ogType)}` : ''}${description ? `&subtitle=${encodeURIComponent(description)}` : ''}`);
</script>
